...
func TcpManage() {
	...
	go func() {
		for _ = range time.Tick(6 * 1e10) {
			ulog.Ul.Debugf("%s:cur con num=%d;dev con in lis=%d;pc con in lis=%d;coded_num=%d;\n", ulog.CreateDateSting(), cur_conn_num, uconnect.CountDevConnList(), uconnect.CountPcConnList(), usql.Uss_coded_num)
			fmt.Printf("%s:cur con num=%d;dev con in lis=%d;pc con in lis=%d;coded_num=%d;\n", ulog.CreateDateSting(), cur_conn_num, uconnect.CountDevConnList(), uconnect.CountPcConnList(), usql.Uss_coded_num)
		}
	}()
