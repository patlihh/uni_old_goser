	...
	if d_len >= 20 {
		...
		if d_len >= int(a.data_len)+6 {
			...
			if a.imei_len > 0 {
				...
				ulog.Ul.Debugf("Cmd0010_UP_INFO not imei!")
				fmt.Printf("Cmd0010_UP_INFO not imei!\n")
			...
			pos++
			if a.hardwareinfo_len > 0 {
				a.hardwartinfo = string(buffer[pos : pos+int(a.hardwareinfo_len)])
				pos += int(a.hardwareinfo_len)
				ulog.Ul.Debugf("Cmd0010_UP_INFO len=%d hardwartinfo=%s", a.hardwareinfo_len, a.hardwartinfo)
