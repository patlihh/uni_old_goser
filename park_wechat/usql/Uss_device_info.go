...
func (p *Cmd10_UP_INFO) InsertUssDeviceInfo() bool {
	...
	if QueryIMEIFromUssDeviceInfo(p.Imei) == true {
		...
		fmt.Println("imei is exit and update")
		return false
	}

	ulog.Ul.Debug("imei no exit, into imei=", p.Imei)
	fmt.Printf("imei no exit, into imei=%s\n", p.Imei)
	...
	ppHis := new(UssSmsRecvHis)
	ppHis.ImeiId = fmt.Sprintf("%s", p.Imei)
	ppHis.DateTime = ulog.CreateDateSting()
	ppHis.PhoneType = p.Phone_type

	ppHis.InsertUssSmsRecvHis()
...
func (p *Cmd10_UP_INFO) CreateUssDeviceInfoTab() {
	...
	fmt.Printf("CreateUssDeviceInfoTab\n")
