...
func CreateUssSmsRecvTable() {
	...
	if cur_month != time.Now().Month() {
		...
		cur_month_table_name := fmt.Sprintf("uss_sms_recv_hisY%dM%02d",
			time.Now().Year(),
			time.Now().Month())

		cur_month_table := fmt.Sprintf("CREATE TABLE IF NOT EXISTS `%s`(`Index` SERIAL PRIMARY KEY,`Imei` VARCHAR(40) NOT NULL,`Time` VARCHAR(30),`PhoneType` VARCHAR(20)) ENGINE=`innodb`,CHARACTER SET=utf8;", cur_month_table_name)

		ulog.Ul.Debug("CreateUssSmsRecvTable cur_month_table=" + cur_month_table)
		...
		insert_str = fmt.Sprintf("INSERT INTO `%s` (Imei,Time,PhoneType) VALUES(?,?,?)", cur_month_table_name)
	...
	if cur_year != time.Now().Year() {
		...
		cur_year_table := fmt.Sprintf("CREATE TABLE IF NOT EXISTS `%s`(`Index` SERIAL PRIMARY KEY,`Imei` VARCHAR(40) NOT NULL,`Time` VARCHAR(30),`PhoneType` VARCHAR(20)) ENGINE=`innodb`,CHARACTER SET=utf8;", cur_year_table_name)

		ulog.Ul.Debug("CreateUssSmsRecvTable cur_year_table=" + cur_year_table)
...
func (pHis *UssSmsRecvHis) InsertUssSmsRecvHis() {
	...
	DbcheckErr(err)
	res, err := stmt.Exec(pHis.ImeiId, pHis.DateTime, pHis.PhoneType)
	DbcheckErr(err)
	id, err := res.LastInsertId()
	DbcheckErr(err)
	ulog.Ul.Debug(id)
